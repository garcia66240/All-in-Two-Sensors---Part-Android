...
import com.xenatronics.sensors2.R;

public class WidgetGauge extends View {
    ...
    public WidgetGauge(Context context) {
        super(context);
        init();
    }
    public WidgetGauge(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WidgetGauge, 0, 0);

        // stroke style
        setStrokeWidth(a.getDimension(R.styleable.WidgetGauge_gaugeStrokeWidth, 10));
        setStrokeColor(a.getColor(R.styleable.WidgetGauge_gaugeStrokeColor, ContextCompat.getColor(context, android.R.color.darker_gray)));
        setStrokeCap(a.getString(R.styleable.WidgetGauge_gaugeStrokeCap));

        // angle start and sweep (opposite direction 0, 270, 180, 90)
        setStartAngle(a.getInt(R.styleable.WidgetGauge_gaugeStartAngle, 0));
        setSweepAngle(a.getInt(R.styleable.WidgetGauge_gaugeSweepAngle, 360));

        // scale (from mStartValue to mEndValue)
        setStartValue(a.getInt(R.styleable.WidgetGauge_gaugeStartValue, 0));
        setEndValue(a.getInt(R.styleable.WidgetGauge_gaugeEndValue, 1000));

        // pointer size and color
        setPointSize(a.getInt(R.styleable.WidgetGauge_gaugePointSize, 0));
        setPointStartColor(a.getColor(R.styleable.WidgetGauge_gaugePointStartColor, ContextCompat.getColor(context, android.R.color.white)));
        setPointEndColor(a.getColor(R.styleable.WidgetGauge_gaugePointEndColor, ContextCompat.getColor(context, android.R.color.white)));

        // divider options
        int dividerSize = a.getInt(R.styleable.WidgetGauge_gaugeDividerSize, 0);
        setDividerColor(a.getColor(R.styleable.WidgetGauge_gaugeDividerColor, ContextCompat.getColor(context, android.R.color.white)));
        int dividerStep = a.getInt(R.styleable.WidgetGauge_gaugeDividerStep, 0);
        setDividerDrawFirst(a.getBoolean(R.styleable.WidgetGauge_gaugeDividerDrawFirst, true));
        setDividerDrawLast(a.getBoolean(R.styleable.WidgetGauge_gaugeDividerDrawLast, true));
